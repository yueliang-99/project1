...
int main()
{
   ...
   char emperorName[50];
   char tempName [20];      //用来存放临时字符串的数组
   char names[MAX][20] = {"貂蝉","杨玉环","不知火舞","西施","小姐姐"};
   //嫔妃的级别数组
   char levelNames[][10] = {"贵人","嫔妃","贵妃","皇贵妃","皇太后"};
   ...
   switch(choice)
   {
    ...
    case 2://翻牌宠幸： \t\t(修改状态功能) \n
        ...
        //3.修改其它妃子的状态  由于羡慕嫉妒恨，其他妃子好感度-10
        //需要解决的bug: 未支持姓名不存在的情况，请自行脑补
        printf("陛下，请输入今天翻牌娘娘的名讳: ");
        scanf("%s",tempName);
        //字符串的查找strcmp,需要引入<string.h>
        //strcmp(tempName,"abc")  0-两个字符串相等； 1表示前一个大于后一个， -1表示后一个大于前一个
        for(i = 0; i< count; i++)
        {
    ...
    case 3:
        ...
        //2.后面一个覆盖给前面一个
        //3.总数--
        //4.修改其它妃子的状态，好感度-20   ******************没写完
         printf("只要你敢不懦弱，凭什么我们要错过。 \n");
         ...
         if(-1 == searchIndex)//如果searchIndex的值胃初值-1，表示没有找到
         {
         ...
         else
         {
            ...
            {
                //names[i] = names[i + 1]; 因为是字符串数组，C语言中不支持数组的直接赋值
                //我们需要使用strcpy函数实现数组的赋值,后续课程会详细讲解，这里只需了解是什么功能即可
